...
async def show_help(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description="Your AI-powered D&D 5e adventure with Donnie the DM's optimized voice!",
        color=0x4169E1
    )
    ...
    embed.add_field(
        name="🚀 Getting Started with Fast Voice",
        value="1. Register character with `/character`\n2. Start adventure with `/start`\n3. Join voice channel yourself\n4. Use `/join_voice` for Donnie to join\n5. Use `/action` - Instant response + queued voice!\n6. Multiple players? No problem - voices won't overlap!",
        inline=False
    )
    
    embed.add_field(
        name="⚡ Speed & Queue Optimizations",
        value="• **Instant Response**: Text appears immediately\n• **Thinking Sounds**: Donnie makes realistic DM sounds while preparing\n• **Voice Queue**: No overlapping with multiple players\n• **Smart Condensing**: Full text + shortened voice\n• **Fast TTS**: 1-2 second voice responses",
        inline=False
    )
    
    embed.set_footer(text="Donnie the DM awaits to guide your fast-paced voice adventure!")
    await interaction.response.send_message(embed=embed)
