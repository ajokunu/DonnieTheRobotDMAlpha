...
voice_speed = {}  # Track speech speed per guild (default 1.15)
...
async def generate_tts_audio(text: str, voice: str = "fable", speed: float = 1.15) -> io.BytesIO:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # OpenAI TTS payload with Fable voice and adjustable speed
            payload = {
...
async def speak_text_in_voice_channel(guild_id: int, text: str):
    ...
    speed = voice_speed.get(guild_id, 1.15)
...
async def join_voice_channel(interaction: discord.Interaction):
    ...
    try:
        ...
        voice_speed[guild_id] = 1.15  # Default faster speed for gameplay
        ...
        welcome_text = "Greetings, brave adventurers! I am Donnie, your Dungeon Master, speaking with OpenAI's expressive Fable voice at optimal speed for gameplay. The giants await your courage!"
...
async def adjust_voice_speed(interaction: discord.Interaction, speed: float):
    ...
    embed.add_field(
        name="🎤 Test It Out",
        value="Use `/test_voice` to hear the new speed, or take an `/action` to hear it in gameplay!",
        inline=False
    )
