...
# Storm King's Thunder DM Prompt (Enhanced for Donnie)
...
- Speak as Donnie the DM - you have a warm, engaging voice perfect for storytelling

PLAYER ACTION: {player_input}

Respond as Donnie, the Storm King's Thunder DM:"""

async def generate_tts_audio(text: str, voice: str = "fable") -> io.BytesIO:
    """Generate TTS audio using OpenAI's Fable voice - expressive and dramatic for storytelling"""
    try:
        # Clean text for TTS (remove excessive formatting)
        clean_text = text.replace("**", "").replace("*", "").replace("_", "")
        
        # Use OpenAI TTS API with Ballad voice
        async with aiohttp.ClientSession() as session:
            ...
            # OpenAI TTS payload with Fable voice and dramatic settings
            payload = {
                "model": "tts-1-hd",  # Use HD model for better quality
                "input": clean_text,
                "voice": voice,  # "fable" voice - expressive and dramatic for storytelling
                "response_format": "mp3",
                "speed": 0.90  # Slower for dramatic, theatrical effect
            }
...
async def speak_text_in_voice_channel(guild_id: int, text: str):
    ...
    # Generate TTS audio
    audio_data = await generate_tts_audio(text, voice="fable")  # Fable voice for dramatic Donnie
    if not audio_data:
        return
...
async def on_ready():
    ...
    try:
        ...
        print("🎲 Storm King's Thunder TTS bot ready for adventure!")
    except Exception as e:
...
async def join_voice_channel(interaction: discord.Interaction):
    ...
    try:
        ...
        welcome_text = "Greetings, brave adventurers! I am Donnie, your Dungeon Master, speaking to you with OpenAI's expressive Fable voice. I'm ready to guide you through the Storm King's Thunder campaign with dramatic, theatrical narration. The giants await your courage!"
        await speak_text_in_voice_channel(guild_id, welcome_text)
...
@bot.tree.command(name="test_voice", description="Test Donnie's voice with a sample message")
async def test_voice(interaction: discord.Interaction):
    """Test TTS functionality"""
    guild_id = interaction.guild.id
    
    if guild_id not in voice_clients or not voice_clients[guild_id].is_connected():
        await interaction.response.send_message("❌ Donnie needs to join a voice channel first! Use `/join_voice`", ephemeral=True)
        return
    
    if not tts_enabled.get(guild_id, False):
        await interaction.response.send_message("❌ Donnie is muted! Use `/unmute_donnie` first.", ephemeral=True)
        return
    
    test_message = "Greetings, adventurers! This is Donnie, your Dungeon Master, speaking with OpenAI's expressive Fable voice. I'm testing my dramatic, theatrical narration to ensure I can properly bring your Storm King's Thunder campaign to life with storytelling flair. The giants tremble at your approach!"
    
    await interaction.response.send_message("🎤 Testing Donnie's dramatic voice...")
    await speak_text_in_voice_channel(guild_id, test_message)
...
"""
    ...
    embed.add_field(
        name="⚡ Next Steps",
        value="Use `/start` to begin, then `/join_voice` to have Donnie narrate your adventure with his dramatic voice!",
        inline=False
    )
...
async def take_action(interaction: discord.Interaction, what_you_do: str):
    """Process player action and get DM response with TTS"""
    user_id = str(interaction.user.id)
    player_name = interaction.user.display_name
    ...
    if guild_id in voice_clients and voice_clients[guild_id].is_connected():
        if tts_enabled.get(guild_id, False):
            embed.add_field(name="🎤", value="*Donnie speaks this response aloud with expressive flair*", inline=False)
        else:
    ...
    # Speak the DM response if TTS is enabled
    if guild_id in voice_clients and tts_enabled.get(guild_id, False):
        await speak_text_in_voice_channel(guild_id, dm_response)
...
async def show_status(interaction: discord.Interaction):
    ...
    guild_id = interaction.guild.id
    if guild_id in voice_clients and voice_clients[guild_id].is_connected():
        voice_status = "🎤 Connected" if tts_enabled.get(guild_id, False) else "🔇 Muted"
    else:
...
async def show_help(interaction: discord.Interaction):
    """Show comprehensive bot guide including TTS features"""
    embed = discord.Embed(
        title="⚡ Storm King's Thunder TTS Bot - Complete Guide",
        description="Your AI-powered D&D 5e adventure with Donnie the DM's dramatic voice!",
        color=0x4169E1
    )
    
    embed.add_field(
        name="🎤 Voice Features (NEW!)",
        value="`/join_voice` - Donnie joins voice to narrate with Fable voice\n`/leave_voice` - Donnie leaves voice channel\n`/mute_donnie` - Disable TTS narration\n`/unmute_donnie` - Enable TTS narration\n`/test_voice` - Test Donnie's expressive voice",
        inline=False
    )
    ...
    embed.add_field(
        name="🚀 Getting Started with Voice",
        value="1. Register character with `/character`\n2. Start adventure with `/start`\n3. Join voice channel yourself\n4. Use `/join_voice` for Donnie to join\n5. Use `/action` - Donnie speaks responses with dramatic flair!",
        inline=False
    )
    
    embed.set_footer(text="Donnie the DM awaits to guide your voice-narrated adventure!")
    await interaction.response.send_message(embed=embed)
